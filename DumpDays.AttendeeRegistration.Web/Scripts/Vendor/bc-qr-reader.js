(function () {
    if (/internet explorer/i.test(window.navigator.userAgent) || /MSIE/i.test(window.navigator.userAgent)) { return; }
    /*
    ...
    */
    ...
    bcQrReader = function ($timeout) {
        return {
            ...
            template: '<div><webcam style="display:none" on-stream="onStream(stream)" on-error="onError(err)" ng-if="active" channel="channel"></webcam><canvas id="qr-canvas" style="width:100%"></canvas></div>',
